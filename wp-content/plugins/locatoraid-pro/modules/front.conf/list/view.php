...
class Front_Conf_List_View_LC_HC_MVC
{
	public function render()
	{
		...
		if( $this_advanced ){
			...
			$links
				->add(
					$this->app->make('/html/ahref')
						->to('/front.conf/list/mode/advanced')
	...
	protected function _render_simple( $inputs_view )
	{
		...
		foreach( $fields as $fn => $flabel ){
			$show_pname = 'front_list:' . $fn . ':show';
			$wlabel_pname = 'front_list:' . $fn . ':w_label';

			$this_row = array();
			$this_row['field'] = $flabel;

			if( isset($inputs_view[$show_pname]) ){
				$this_row['show_in_list'] = $inputs_view[$show_pname];
			}
