...
class PluginUpdateChecker_1_5 {
	...
	protected function installHooks(){
		...
		if ( $this->checkPeriod > 0 ){
			...
			if ( ($this->checkPeriod >= 1) && array_key_exists($this->checkPeriod, $defaultSchedules) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if( isset($state->update->upgrade_notice) && $state->update->upgrade_notice )
			{
				$class = 'error';
				$message = $state->update->upgrade_notice;
			}
			else
			{
				$class = 'updated';
